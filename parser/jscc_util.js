  ...
  parse(buf, onerror) {
    ...
    function get_act(top, la) {
      for (let i = 0; i < act_tab[top].length; i += 2) {
        if (act_tab[top][i] === la) return act_tab[top][i + 1]
      }
    ...
    function get_goto(top, pop) {
      for (let i = 0; i < goto_tab[top].length; i += 2) {
        if (goto_tab[top][i] === pop) return goto_tab[top][i + 1]
      }
      return null
    }
    ...
    while (1) {
      ...
      if (PCB.act === null && defact_tab[sstack.bottom] >= 0) PCB.act = -defact_tab[sstack.bottom]
      if (PCB.act === null) {
