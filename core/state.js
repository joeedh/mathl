...
export class ParseState {
  constructor(source, filename = '(anonymous)', parser, preprocessed = '', oldState) {
  ...
  loadOldState(state) {
    this.poly_keymap = {...state.poly_keymap, ...this.poly_keymap}
    this.poly_namemap = {...state.poly_namemap, ...this.poly_namemap}
    this.functions = {...state.functions, ...this.functions}
    this.inputs = {...state.inputs, ...this.inputs}
  ...
  addLibraryFunc(node) {
    const args = node[1].map((arg) => arg[0].value)
    const retType = node[0]

    if (this.isPolyKey(node.value)) {
      this.addPolyFunc(this.getPolyKeyName(node.value), retType, args)
    }
  }
  ...
  buildPolyKey(name, rtype, args, type2) {
    ...
    let key = `_$_${name}__${rtype.getTypeNameSafe()}__`
...
export function popParseState(options = {keepPolyFuncs: false, keepFuncs: false}) {
  let old = state
  ...
  if (options?.keepPolyFuncs) {
    ...
    for (const k in old.functions) {
      if (options?.keepFuncs || (state.isPolyKey(k) && options?.keepPolyFuncs)) {
        state.functions[k] = old.functions[k]
      }
    }
  }
}
