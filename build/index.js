      ...
      makeMap = /* @__PURE__ */ __name(function (name, relName) {
        ...
        if (prefix) {
          if (plugin && plugin.normalize) {
            name =
              /** @type {{normalize: function(string, function(string):string):string}} */
              plugin.normalize(name, makeNormalize(relName))
          } else {
...
`
    ...
    ;(function (root, factory) {
      ...
      function (require3, others) {
        var SYM = require3('../enums/SYM'),
          ASSOC = require3('../enums/ASSOC'),
          SPECIAL = require3('../enums/SPECIAL')
    ...
    ;(function (root, factory) {
      ...
      function (require3, others) {
        var defaultParserDriver,
          ...
          SPECIAL = require3('./enums/SPECIAL'),
          EXEC = require3('./enums/EXEC')
        ;(function () {
          if (false) {
            try {
              defaultParserDriver = /** @type {string} */ require3('text!./template/parser-driver-js.txt')
            } catch (e) {
              if (typeof fs !== 'undefined' && typeof path !== 'undefined') {
                ...
                defaultParserDriver =
                  /** @type {string} */
                  require3('fs').readFileSync(
                    require3('path').join(__dirname, 'template', 'parser-driver-js.txt'),
                    'utf8'
                  )
              }
            }
    ...
    ;(function (root, factory) {
      ...
      jscc.ioBrowser.prototype.write_output = function (options) {
        if (typeof global2.write_output_function === 'function') {
          var text = ''
          var callback = /* @__PURE__ */ __name(function () {}, 'callback')
    ...
    ;(function (root, factory) {
      ...
      function (require3, others) {
        ...
        ;(function () {
          if (false) {
            io = /** @type {jscc.io} */ require3('./io/ioNode')
          } else {
            io = /** @type {jscc.io} */ require3('./io/io')
          }
        })()
        jscc.debug = function () {}
              ...
              if (mode == MODE_GEN.HTML) {
                ...
                for (j = 0; j < global2.productions[item_set[i2].prod].rhs.length; j++) {
                  ...
                  if (global2.symbols[global2.productions[item_set[i2].prod].rhs[j]].kind == SYM.TERM) {
                    io.write_debug(
                      '<b>' + global2.symbols[global2.productions[item_set[i2].prod].rhs[j]].label + '</b>'
                    )
                  } else {
    ...
    ;(function (root, factory) {
      ...
      function (require3, others) {
        ...
        jscc.tabgen.prototype.find_symbol = function (label, kind, special) {
          ...
          for (var i2 = 0; i2 < global2.symbols.length; i2++) {
            if (
              global2.symbols[i2].label.toString() == label.toString() &&
              global2.symbols[i2].kind == kind &&
              global2.symbols[i2].special == special
            ) {
              return i2
            }
          }
          ...
          for (i2 = 0; i2 < seed.length; i2++) {
            if (seed[i2].dot_offset < global2.productions[seed[i2].prod].rhs.length) {
              if (global2.symbols[global2.productions[seed[i2].prod].rhs[seed[i2].dot_offset]].kind == SYM.NONTERM) {
                for (
                  j = 0;
                  j < global2.symbols[global2.productions[seed[i2].prod].rhs[seed[i2].dot_offset]].prods.length;
                  j++
                ) {
        ...
        jscc.tabgen.prototype.lalr1_closure = function (s) {
          ...
          for (i2 = 0; i2 < global2.states[s].kernel.length; i2++) {
            if (
              global2.states[s].kernel[i2].dot_offset <
              global2.productions[global2.states[s].kernel[i2].prod].rhs.length
            ) {
              closure.unshift(
                new Item({
          ...
          for (i2 = 0; i2 < closure.length; i2++) {
            if (global2.productions[closure[i2].prod].rhs.length == 0) {
              for (j = 0; j < global2.states[s].epsilon.length; j++) {
                if (
                  global2.states[s].epsilon[j].prod == closure[i2].prod &&
                  global2.states[s].epsilon[j].dot_offset == closure[i2].dot_offset
                ) {
                  break
                }
              }
              ...
              if (!global2.states[s].closed) {
                for (j = 0; j < partition.length; j++) {
                  if (partition[j].dot_offset - 1 < global2.productions[partition[j].prod].rhs.length) {
                    if (
                      global2.symbols[global2.productions[partition[j].prod].rhs[partition[j].dot_offset - 1]].kind ==
                      SYM.TERM
                    ) {
              ...
              if (item_set[i2].dot_offset == global2.productions[item_set[i2].prod].rhs.length) {
                for (j = 0; j < item_set[i2].lookahead.length; j++) {
                  ...
                  if (ex == void 0) {
                    ...
                    if (ex > 0) {
                      if (
                        global2.symbols[item_set[i2].lookahead[j]].level > 0 ||
                        global2.productions[item_set[i2].prod].level > 0
                      ) {
                        if (
                          global2.symbols[item_set[i2].lookahead[j]].level ==
                          global2.productions[item_set[i2].prod].level
                        ) {
                          if (global2.symbols[item_set[i2].lookahead[j]].associativity == ASSOC.LEFT) {
                            ...
                            log3.warn(
                              "Removing nonassociative symbol '" +
                                global2.symbols[item_set[i2].lookahead[j]].label +
                                "' in state " +
                                s
                            )
    ...
    ;(function (root, factory) {
      ...
      function (require3, others) {
        ...
        ;(function () {
          if (false) {
            log3 = /** @type {jscc.log} */ require3('./log/logNode')
          } else {
            log3 = /** @type {jscc.log} */ require3('./log/log')
          }
        })()
              ...
              for (j = 0, k = 0; j < src.length; j++, k++) {
                ...
                if (strmatch && strmatch.index == 0) {
                  if (strmatch[0] == '%%') {
                    ...
                    if (idx < 0) {
                      var badProduction = global2.productions[i2],
                        badLeftSymbol = global2.symbols[badProduction.lhs]
                      if (global2.productions[i2].rhs.length == 0) {
                        ...
                        log3.error(
                          'The wildcard ' +
                            ...
                            global2.productions[i2].rhs.length +
                            " symbols on the right-hand side of the production.  The faulty left-hand side symbol label is '" +
                            badLeftSymbol.label +
                            "'."
                        )
                      }
          ...
          print_nfa: function () {
            ...
            for (var i2 = 0; i2 < global2.nfa_states.value.length; i2++) {
              io.write_debug(
                i2 +
                  '	' +
                  (global2.nfa_states.value[i2].edge == EDGE.FREE
                    ? 'FREE'
                    : global2.nfa_states.value[i2].edge == EDGE.EPSILON
                      ? 'EPSILON'
                      : 'CHAR') +
                  '		' +
                  (global2.nfa_states.value[i2].edge != EDGE.FREE && global2.nfa_states.value[i2].follow > -1
                    ? global2.nfa_states.value[i2].follow
                    : '') +
                  '		' +
                  (global2.nfa_states.value[i2].edge != EDGE.FREE && global2.nfa_states.value[i2].follow2 > -1
                    ? global2.nfa_states.value[i2].follow2
                    : '') +
                  '		' +
                  (global2.nfa_states.value[i2].edge != EDGE.FREE && global2.nfa_states.value[i2].accept > -1
                    ? global2.nfa_states.value[i2].accept
                    : '')
              )
      ...
      function (require3, others) {
        ...
        var __parse = (function (eof, whitespace, error_token) {
            ...
            var actions = [
              ...
              function () {
                ...
                for (; i2 < arguments[1].length; i2++) {
                  ...
                  } else global2.nfa_states.value[rval.start].ccl.set(arguments[1].charCodeAt(i2), !negate)
                }
                return rval
              },
              function () {
          ...
          function get_act(top, la) {
            for (var i2 = 0; i2 < act_tab[top].length; i2 += 2) if (act_tab[top][i2] === la) return act_tab[top][i2 + 1]
            return null
          }
          __name(get_act, 'get_act')
          function get_goto(top, pop) {
            for (var i2 = 0; i2 < goto_tab[top].length; i2 += 2)
              if (goto_tab[top][i2] === pop) return goto_tab[top][i2 + 1]
            return null
          }
          ...
          function parse3(src, err_off, err_la) {
            ...
            while (true) {
              ...
              if (PCB2.act === null && defact_tab[sstack[0]] >= 0) PCB2.act = -defact_tab[sstack[0]]
              if (PCB2.act === null) {
                if (PCB2.error_step === 0) {
                  err_cnt++
                  err_off.unshift(PCB2.offset - PCB2.att.length)
                  err_la.unshift([])
                  for (i2 = 0; i2 < act_tab[sstack[0]].length; i2 += 2) err_la[0].push(labels[act_tab[sstack[0]][i2]])
                }
                ...
                if (sstack.length > 1 && PCB2.act !== null) {
                  while (PCB2.la !== eof) {
                    ...
                    if (PCB2.act != null) break
                    while (PCB2.lex() != null) PCB2.offset++
                  }
                }
              ...
              if (PCB2.act > 0) {
                ...
                PCB2.lex()
                if (PCB2.error_step > 0) PCB2.error_step--
              } else {
                act = -PCB2.act
        ...
        function compile_regex(str, accept, case_insensitive, cur_line) {
          ...
          if ((error_count = __parse(str, error_offsets, error_expects)) == 0) {
            ...
            if (first_nfa > 0) {
              i2 = 0
              while (global2.nfa_states.value[i2].follow2 != -1) i2 = global2.nfa_states.value[i2].follow2
              global2.nfa_states.value[i2].follow2 = first_nfa
            }
          } else {
            for (i2 = 0; i2 < error_count; i2++) {
              var spaces = ''
              for (j = 0; j < error_offsets[i2]; j++) spaces += ' '
              log3.error(
                'Regular expression:\n	' +
                  str +
                  '\n	' +
    ...
    ;(function (root, factory) {
      ...
      function (require3, others) {
        ...
        var __parse = (function (eof, whitespace, error_token) {
          ...
          var defact_tab = [
            35, 0, -1, 2, 32, -1, 37, -1, 5, -1, -1, -1, -1, -1, 12, 35, 38, 39, 36, 34, 33, 40, 4, -1, -1, -1, -1, 11,
            9, 42, 14, 35, 35, 16, -1, -1, 6, 7, 8, 10, 41, 13, 15, 1, 3, 26, 18, -1, 20, 24, 25, 28, 29, 30, 31, 26,
            17, 35, -1, 27, 19, 21, 23, 22,
          ]
          var labels = [
            {
              label   : "def'",
          ...
          var ACTIONS = /* @__PURE__ */ (function () {
            ...
            var actions = [
              ...
              function () {
                ...
                } else line_error(PCB2.line, 'Multiple whitespace definition')
                return rval
              },
              function () {
                var rval
              ...
              function () {
                ...
                else line_error(PCB2.line, 'Call to undefined terminal "' + arguments[0] + '"')
                return rval
              },
              function () {
                var rval
            ...
            for (var i2 = 0; i2 < parse_error; i2++)
              line_error(
                str.substr(0, error_offsets[i2]).match(/\n/g)
                  ...
                  : 1,
                'Parse error near\n	' +
                  str.substr(error_offsets[i2], 30) +
                  (error_offsets[i2] + 30 < str.substr(error_offsets[i2]).length ? '...' : '') +
                  '\n	' +
                  error_expects[i2].join() +
                  ' expected'
              )
          ...
          create_subset: function (nfa_states) {
            ...
            while ((current = this.get_undone_dfa(dfa_states)) > -1) {
              ...
              for (var i2 = 0; i2 < dfa_states[current].nfa_set.length; i2++) {
                if (
                  (nfa_states[dfa_states[current].nfa_set[i2]].accept > -1 &&
                    nfa_states[dfa_states[current].nfa_set[i2]].weight < lowest_weight) ||
                  lowest_weight == -1
                ) {
...
var parser3 = class {
  ...
  error(token4, msg) {
    let estr
    if (msg === void 0) msg = ''
    if (token4 === void 0) estr = 'Parse error at end of input: ' + msg
    else estr = 'Parse error at line ' + (token4.lineno + 1) + ': ' + msg
    let buf = '1| '
...
function StructParser() {
  ...
  let tokens3 = [
    ...
    tk3('SPACE', / |\t/, function (t) {}, 'whitespace'),
  ]
...
function setDebugMode(t) {
  ...
  if (debug) {
    ...
    packer_debug = /* @__PURE__ */ __name(function () {}, 'packer_debug')
    packer_debug_start = /* @__PURE__ */ __name(function () {}, 'packer_debug_start')
    packer_debug_end = /* @__PURE__ */ __name(function () {}, 'packer_debug_end')
  }
}
...
var StructIterKeysField = class extends StructFieldType {
  ...
  static pack(manager2, data, val2, obj, field, type) {
    ...
    for (let val22 in val2) {
      if (i2 >= len) {
        if (warninglvl > 0) console.warn('Warning: object keys magically replaced on us', val2, i2)
        return
      }
...
function define_empty_class(name) {
  let cls = /* @__PURE__ */ __name(function () {}, 'cls')
...
var STRUCT = class _STRUCT {
  ...
  static Super(obj, reader) {
    if (warninglvl$1 > 0) console.warn('deprecated')
    reader(obj)
  ...
  static chain_fromSTRUCT(cls, reader) {
    if (warninglvl$1 > 0) console.warn('Using deprecated (and evil) chain_fromSTRUCT method, eek!')
    ...
    for (let i2 = 0; i2 < keys2.length; i2++) {
      ...
      try {
        ...
        if (warninglvl$1 > 0) console.warn('  failed to set property', k)
      }
    }
    return obj2
  }
  ...
  parse_structs(buf, defined_classes) {
    ...
    for (let i2 = 0; i2 < defined_classes.length; i2++) {
      ...
      if (!cls.structName && cls.STRUCT) {
        ...
        if (warninglvl$1 > 0) console.log('Warning, bad class in registered class list', unmangle(cls.name), cls)
        continue
      }
    ...
    while (!struct_parse.at_end()) {
      ...
      if (!(stt.name in clsmap)) {
        if (!(stt.name in this.null_natives)) {
          if (warninglvl$1 > 0) console.log('WARNING: struct ' + stt.name + ' is missing from class list.')
        }
  ...
  _env_call(code3, obj, env) {
    ...
    let fullcode = ''
    if (envcode !== _static_envcode_null$1) fullcode = envcode + code3
    else fullcode = code3
    let func
...
var LZString = (function () {
  ...
  var LZString2 = {
    ...
    compressToUTF16: function (input) {
      if (input == null) return ''
      return (
        LZString2._compress(input, 15, function (a) {
          return f(a + 32)
        }) + ' '
      )
    },
...
export {GLSLValueType, compileJS, findSlots, genCode, genJS, parse2 as parse, preprocess, silence, unsilence}
/**
