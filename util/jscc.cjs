...
;(function (undef) {
  ...
  function normalize(name, baseName) {
    var /** @type {Array<string>} */ nameParts,
      nameSegment,
      mapValue,
      foundMap,
      lastIndex,
      ...
      /** @type {almondRequire} */ (
        function (deps, callback, relName, forceSync, alt) {
          ...
          if (forceSync) {
            main(
              undef,
              /** @type {(Array<string>|undefined)} */ (deps),
              callback,
              /** @type {(string|undefined)} */ (relName)
            )
          } else {
...
;(function (root, factory) {
  ...
  jscc.BitSet32 = function () {
    ...
    this.set = function (bit, state) {
      state = !!state
      that._data[bit >> 5] = state
        ? that._data[bit >> 5] | (1 << (bit & 31))
        : that._data[bit >> 5] & ~(1 << (bit & 31))
      return state
    }
    /**
...
;(function (root, factory) {
  ...
  function (require, others) {
    ...
    jscc.classes.Nfa = function (o) {
      ...
      if (
        typeof p.ccl === 'object' &&
        p.ccl.hasOwnProperty('get') &&
        p.ccl.hasOwnProperty('set') &&
      ...
      if (false) {
        try {
          ...
          if (typeof fs !== 'undefined' && typeof path !== 'undefined') {
            ...
            defaultParserDriver =
              /** @type {string} */
              (require('fs').readFileSync(require('path').join(__dirname, 'template', 'parser-driver-js.txt'), 'utf8'))
          }
        }
      } else {
        defaultParserDriver = /** @type {string} */ (require('text!./template/parser-driver-js.txt'))
          ...
          if (global.symbols[i].kind == SYM.NONTERM) {
            for (var j = 0; j < global.symbols[i].prods.length; j++) {
              ...
              for (var k = 0; k < global.productions[global.symbols[i].prods[j]].rhs.length; k++) {
                global.symbols[i].first = util.union(
                  global.symbols[i].first,
                  global.symbols[global.productions[global.symbols[i].prods[j]].rhs[k]].first
                )

                nullable = global.symbols[global.productions[global.symbols[i].prods[j]].rhs[k]].nullable
                if (!nullable) {
                  break
                }
              }
        ...
        if (mode == MODE_GEN.HTML) {
          ...
          for (var i = 0; i < global.symbols.length; i++) {
            ...
            if (global.symbols[i].kind == SYM.NONTERM) {
              ...
              for (var j = 0; j < global.symbols[i].prods.length; j++) {
                for (var k = 0; k < global.productions[global.symbols[i].prods[j]].rhs.length; k++) {
                  if (global.symbols[global.productions[global.symbols[i].prods[j]].rhs[k]].kind == SYM.TERM) {
                    io.write_debug(
                      '<b>' + global.symbols[global.productions[global.symbols[i].prods[j]].rhs[k]].label + '</b>'
                    )
                  } else {
                    io.write_debug(
          ...
          for (var i = 0; i < global.symbols.length; i++) {
            if (global.symbols[i].kind == SYM.NONTERM) {
              ...
              for (var j = 0; j < global.symbols[i].prods.length; j++) {
                ...
                for (var k = 0; k < global.productions[global.symbols[i].prods[j]].rhs.length; k++) {
                  if (global.symbols[global.productions[global.symbols[i].prods[j]].rhs[k]].kind == SYM.TERM) {
                    output += '#' + global.symbols[global.productions[global.symbols[i].prods[j]].rhs[k]].label + ' '
                  } else {
                    output += global.symbols[global.productions[global.symbols[i].prods[j]].rhs[k]].label + ' '
                  }
                }
          ...
          if (mode == MODE_GEN.HTML) {
            ...
            for (j = 0; j < global.productions[item_set[i].prod].rhs.length; j++) {
              ...
              if (global.symbols[global.productions[item_set[i].prod].rhs[j]].kind == SYM.TERM) {
                io.write_debug('<b>' + global.symbols[global.productions[item_set[i].prod].rhs[j]].label + '</b>')
              } else {
                io.write_debug(' ' + global.symbols[global.productions[item_set[i].prod].rhs[j]].label + ' ')
              }
            }
...
;(function (root, factory) {
  ...
  function (require, others) {
    ...
    var TableEntry = /** @type {function(new:jscc.classes.TableEntry, number, number)} */ (
      require('./classes/TableEntry')
    )
    ...
    jscc.tabgen.prototype.find_symbol = function (label, kind, special) {
      ...
      for (var i = 0; i < global.symbols.length; i++) {
        if (
          global.symbols[i].label.toString() == label.toString() &&
    ...
    jscc.tabgen.prototype.item_set_equal = function (set1, set2) {
      ...
      for (i = 0; i < set1.length; i++) {
        for (j = 0; j < set2.length; j++) {
          if (set1[i].prod == set2[j].prod && set1[i].dot_offset == set2[j].dot_offset) {
            cnt++
            break
          }
        }
    ...
    jscc.tabgen.prototype.close_items = function (seed, closure) {
      ...
      for (i = 0; i < seed.length; i++) {
        if (seed[i].dot_offset < global.productions[seed[i].prod].rhs.length) {
          if (global.symbols[global.productions[seed[i].prod].rhs[seed[i].dot_offset]].kind == SYM.NONTERM) {
            for (
              j = 0;
              ...
              for (k = 0; k < closure.length; k++) {
                if (
                  closure[k].prod == global.symbols[global.productions[seed[i].prod].rhs[seed[i].dot_offset]].prods[j]
                ) {
                  break
                }
              }
    ...
    jscc.tabgen.prototype.lalr1_closure = function (s) {
      ...
      for (i = 0; i < global.states[s].kernel.length; i++) {
        if (global.states[s].kernel[i].dot_offset < global.productions[global.states[s].kernel[i].prod].rhs.length) {
          closure.unshift(
            new Item({
      ...
      for (i = 0; i < closure.length; i++) {
        if (global.productions[closure[i].prod].rhs.length == 0) {
          for (j = 0; j < global.states[s].epsilon.length; j++) {
            if (
              global.states[s].epsilon[j].prod == closure[i].prod &&
      ...
      while (closure.length > 0) {
        ...
        if (partition.length > 0) {
          ...
          for (j = 0; j < partition.length; j++) {
            ...
            global.states[i].kernel[j].lookahead = util.union(
          ...
          if (!global.states[s].closed) {
            for (j = 0; j < partition.length; j++) {
              if (partition[j].dot_offset - 1 < global.productions[partition[j].prod].rhs.length) {
                if (
                  global.symbols[global.productions[partition[j].prod].rhs[partition[j].dot_offset - 1]].kind ==
                  SYM.TERM
                ) {
                  global.states[s].actionrow = this.add_table_entry(
                    global.states[s].actionrow,
                    global.productions[partition[j].prod].rhs[partition[j].dot_offset - 1],
                    i
                  )
                  global.shifts++
                } else {
                  global.states[s].gotorow = this.add_table_entry(
                    global.states[s].gotorow,
                    global.productions[partition[j].prod].rhs[partition[j].dot_offset - 1],
                    i
                  )
                  global.gotos++
        ...
        for (i = 0; i < item_set.length; i++) {
          if (item_set[i].dot_offset == global.productions[item_set[i].prod].rhs.length) {
            for (j = 0; j < item_set[i].lookahead.length; j++) {
              ...
              ex = this.get_table_entry(global.states[s].actionrow, item_set[i].lookahead[j])
              ...
              if (ex == void 0) {
                ...
                if (ex > 0) {
                  ...
                  if (
                    global.symbols[item_set[i].lookahead[j]].level > 0 ||
                    global.productions[item_set[i].prod].level > 0
                  ) {
                    // Is the level the same?
                    if (global.symbols[item_set[i].lookahead[j]].level == global.productions[item_set[i].prod].level) {
                      // In case of left-associativity, reduce
                      if (global.symbols[item_set[i].lookahead[j]].associativity == ASSOC.LEFT) {
                        // Reduce
                        act = -1 * item_set[i].prod
                      } else if (global.symbols[item_set[i].lookahead[j]].associativity == ASSOC.NOASSOC) {
                        // else, if nonassociativity is set,
                        // remove table entry
                        this.remove_table_entry(global.states[s].actionrow, item_set[i].lookahead[j])
                        log.warn(
                  ...
                  act = act * -1 < item_set[i].prod ? act : -1 * item_set[i].prod

                  warning = 'Reduce'
                }

                warning +=
                  "-reduce conflict on symbol '" + global.symbols[item_set[i].lookahead[j]].label + "' in state " + s
                warning +=
                ...
                if (act != ex) {
                  this.update_table_entry(global.states[s].actionrow, item_set[i].lookahead[j], act)
                }
              }
            }
          }
      ...
      if (debug) {
        for (i = 0; i < global.states.length; i++) {
          debugFunctions.print_item_set(
            global.exec_mode == EXEC.CONSOLE ? MODE_GEN.TEXT : MODE_GEN.HTML,
            'states[' + i + '].kernel',
            global.states[i].kernel
          )
          debugFunctions.print_item_set(
            global.exec_mode == EXEC.CONSOLE ? MODE_GEN.TEXT : MODE_GEN.HTML,
            'states[' + i + '].epsilon',
            global.states[i].epsilon
          )
        }
        ...
        switch (mode) {
          ...
          case 'html':
            ...
            for (i = 0; i < global.productions.length; i++) {
              code += '<tr>'
              code += '<td style="border-right: 1px solid lightgray;">' + global.productions[i].lhs + '</td>'
            ...
            for (i = 0; i < global.states.length; i++) {
              ...
              code += '<td>' + (global.states[i].def_act < 0 ? '(none)' : global.states[i].def_act) + '</td>'
              code += '</tr>'
            }
            code += '</table>'
            break
          ...
          case 'js':
            ...
            for (i = 0; i < global.states.length; i++) {
              ...
              for (j = 0; j < global.states[i].actionrow.length; j++) {
                act_tab_json_item.push(global.states[i].actionrow[j].symbol, global.states[i].actionrow[j].action)
              }
            ...
            for (i = 0; i < global.states.length; i++) {
              ...
              for (j = 0; j < global.states[i].gotorow.length; j++) {
                goto_tab_json_item.push(global.states[i].gotorow[j].symbol, global.states[i].gotorow[j].action)
              }
...
;(function (root, factory) {
  ...
  function (require, others) {
    ...
    jscc.integrity.prototype = {
      ...
      undef: function () {
        for (var i = 0; i < global.symbols.length; i++) {
          if (global.symbols[i].kind == SYM.NONTERM && global.symbols[i].defined == false) {
            log.error('Call to undefined non-terminal "' + global.symbols[i].label + '"')
          }
        }
      },
        ...
        while (stack.length > 0) {
          ...
          for (j = 0; j < global.symbols[i].prods.length; j++) {
            for (k = 0; k < global.productions[global.symbols[i].prods[j]].rhs.length; k++) {
              if (global.symbols[global.productions[global.symbols[i].prods[j]].rhs[k]].kind == SYM.NONTERM) {
          ...
          io.write_debug(
            i +
              ...
              (global.nfa_states.value[i].edge == EDGE.FREE
                ? 'FREE'
                : global.nfa_states.value[i].edge == EDGE.EPSILON
                  ? 'EPSILON'
                  : 'CHAR') +
              '\t\t' +
...
;(function (root, factory) {
  ...
  function (require, others) {
    ...
    var __parse = (function (/** number */ eof, /** number */ whitespace, /** number */ error_token) {
      ...
      var ACTIONS = (function () {
        ...
        var actions = [
          ...
          function () {
            ...
            rval.end = global.nfa_states.value[rval.start].follow = global.nfa_states.create()
          ...
          function () {
            ...
            if (arguments[1].charAt(i) == '^') {
              negate = true
              for (j = global.MIN_CHAR; j < global.MAX_CHAR; j++) global.nfa_states.value[rval.start].ccl.set(j, true)
              i++
            }
            for (; i < arguments[1].length; i++) {
              if (arguments[1].charAt(i + 1) == '-' && i + 2 < arguments[1].length) {
                i++
                for (j = arguments[1].charCodeAt(i - 1); j < arguments[1].charCodeAt(i + 1); j++)
                  global.nfa_states.value[rval.start].ccl.set(j, !negate)
              } else global.nfa_states.value[rval.start].ccl.set(arguments[1].charCodeAt(i), !negate)
            }
          ...
          function () {
            ...
            rval.end = global.nfa_states.value[rval.start].follow = global.nfa_states.create()
            global.nfa_states.value[rval.start].edge = EDGE.CHAR
            for (var i = global.MIN_CHAR; i < global.MAX_CHAR; i++) global.nfa_states.value[rval.start].ccl.set(i, true)
       ...
       */
      function get_act(top, la) {
        for (var i = 0; i < act_tab[top].length; i += 2) if (act_tab[top][i] === la) return act_tab[top][i + 1]
        return null
      }
      function get_goto(top, pop) {
        for (var i = 0; i < goto_tab[top].length; i += 2) if (goto_tab[top][i] === pop) return goto_tab[top][i + 1]
        return null
      }
        ...
        while (loop) {
          ...
          do {
            ...
            do {
              chr = this.src.charCodeAt(pos)
              dfa.exec(chr, pos)
              if (dfa.state !== null) this.accountChar(chr)
              pos++
            } while (dfa.state !== null)
          } while (whitespace > -1 && dfa.match === whitespace)
          if (dfa.match !== null) {
      ...
      function parse(/** string */ src, err_off, err_la) {
        ...
        while (true) {
          PCB.act = get_act(sstack[0], PCB.la)
          if (PCB.act === null && defact_tab[sstack[0]] >= 0) PCB.act = -defact_tab[sstack[0]]
          if (PCB.act === null) {
            ...
            if (PCB.error_step === 0) {
              ...
              err_off.unshift(PCB.offset - PCB.att.length)
              err_la.unshift([])
              for (i = 0; i < act_tab[sstack[0]].length; i += 2) err_la[0].push(labels[act_tab[sstack[0]][i]])
            }
            //Perform error recovery
            while (sstack.length > 1 && PCB.act === null) {
              sstack.shift()
    ...
    function compile_regex(str, accept, case_insensitive, cur_line) {
      ...
      if ((error_count = __parse(str, error_offsets, error_expects)) == 0) {
		...
		*/
        ...
        if (first_nfa > 0) {
          i = 0
          while (global.nfa_states.value[i].follow2 != -1) i = global.nfa_states.value[i].follow2
        ...
        for (i = 0; i < error_count; i++) {
          var spaces = ''
          for (j = 0; j < error_offsets[i]; j++) spaces += ' '
...
;(function (root, factory) {
  ...
  function (require, others) {
    var log,
      ...
      tabgen = /** @type {jscc.tabgen} */ (require('./tabgen')),
      Production = /** @type {function(new:jscc.classes.Production, ?ProductionOptions=)} */ (
    ...
    var __parse = (function (/** number */ eof, /** number */ whitespace, /** number */ error_token) {
      ...
      var TERMINAL_ACTIONS = (function () {
        ...
        var actions = {
          13: /** @suppress {uselessCode} */ function (PCB) {
            return PCB.att.substr(2, PCB.att.length - 4)
            return PCB.att
          },
          17: /** @suppress {uselessCode} */ function (PCB) {
            return Continue.apply(null, arguments)
            return PCB.att
          },
          18: /** @suppress {uselessCode} */ function (PCB) {
            return Continue.apply(null, arguments)
            return PCB.att
          },
          19: /** @suppress {uselessCode} */ function (PCB) {
            return Continue.apply(null, arguments)
            return PCB.att
          },
        }
      ...
      var defact_tab = [
        35, 0, -1, 2, 32, -1, 37, -1, 5, -1, -1, -1, -1, -1, 12, 35, 38, 39, 36, 34, 33, 40, 4, -1, -1, -1, -1, 11, 9,
        42, 14, 35, 35, 16, -1, -1, 6, 7, 8, 10, 41, 13, 15, 1, 3, 26, 18, -1, 20, 24, 25, 28, 29, 30, 31, 26, 17, 35,
        -1, 27, 19, 21, 23, 22,
      ]
      ...
      var ACTIONS = (function () {
        ...
        var actions = [
          ...
          function () {
            var rval
            global.code_head += arguments[0]
            return rval
          },
          function () {
            var rval
            global.code_foot += arguments[0]
            return rval
          },
          function () {
            var rval
          ...
          function () {
            ...
            } else line_error(PCB.line, 'Multiple whitespace definition')
            ...
            if (prod.level == 0) {
              if (prod.rhs.length > 0)
                for (var i = prod.rhs.length - 1; i >= 0; i--)
                  if (global.symbols[prod.rhs[i]] && global.symbols[prod.rhs[i]].kind == SYM.TERM) {
          ...
          function () {
            ...
            else line_error(PCB.line, 'Call to undefined terminal "' + arguments[0] + '"')
      ...
      create_subset: function (nfa_states) {
        ...
        while ((current = this.get_undone_dfa(dfa_states)) > -1) {
          ...
          for (var i = 0; i < dfa_states[current].nfa_set.length; i++) {
            if (
              (nfa_states[dfa_states[current].nfa_set[i]].accept > -1 &&
            ...
            if (groups[i].length > 0) {
              for (j = 1; j < groups[i].length; j++) {
                for (k = global.MIN_CHAR; k < global.MAX_CHAR; k++) {
                  ...
                  if (
                    (groupZeroLineK != groupJLineK && (groupZeroLineK == -1 || groupJLineK == -1)) ||
                    (groupZeroLineK > -1 &&
                      groupJLineK > -1 &&
                      dfa_states[groupZeroLineK].group != dfa_states[groupJLineK].group)
                  ) {
