...
class parser {
  ...
  error(token, msg) {
    ...
    if (token === undefined) estr = 'Parse error at end of input: ' + msg
    else estr = 'Parse error at line ' + (token.lineno + 1) + ': ' + msg
...
function StructParser() {
  ...
  let tokens = [
    ...
    tk('SPACE', / |\t/, function (t) {}, 'whitespace'),
  ]
...
class StructIterKeysField extends StructFieldType {
  static pack(manager, data, val, obj, field, type) {
    ...
    for (let val2 in val) {
      if (i >= len) {
        if (warninglvl > 0) console.warn('Warning: object keys magically replaced on us', val, i)
        return
      }
...
class STRUCT {
  ...
  static Super(obj, reader) {
    ...
    if (!bad && parent.prototype.loadSTRUCT && parent.prototype.loadSTRUCT !== obj.loadSTRUCT) {
      //parent.prototype.hasOwnProperty("loadSTRUCT")) {
  ...
  _env_call(code, obj, env) {
    ...
    let fullcode = ''
    if (envcode !== _static_envcode_null$1) fullcode = envcode + code
    else fullcode = code
    let func
...
export {
  ...
  register,
  setAllowOverriding,
  setDebugMode$1 as setDebugMode,
  setEndian$1 as setEndian,
  setTruncateDollarSign,
  setWarningMode$1 as setWarningMode,
  truncateDollarSign$1 as truncateDollarSign,
